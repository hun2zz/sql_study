...
-- 이 나라가 속한 부서의 모든 사원정보 조회
-- 질문 1 이나라는 부서코드가 몇변이냐 ?
-- 질문 2 그 부서코드는 모든 사원을 조회해라.
...
WHERE dept_cd = (
    SELECT
        dept_cd
    FROM tb_emp
    WHERE emp_nm = '이관심' -- 이관심은 부서코드가 2개가 나오기때문에 단일행이 아님.
    )
;
...
--				만족하는 값이 하나라도 존재하면 참
    ...
    dept_cd
FROM tb_emp
WHERE dept_cd IN ( -- = 을 IN으로 바꾸면 정상 출력.
    SELECT
        dept_cd
    FROM tb_emp
    WHERE emp_nm = '이관심' 
...
SELECT
    ...
    dept_nm
FROM tb_dept D
WHERE EXISTS( -- exists는 매칭 된 얘들의 카운트를 세라. 여기서는
                -- 강남이 매칭이고 D.dept_cd와 E.dept_cd가 같아야함.
    SELECT
        D.dept_cd
    FROM tb_emp E
    WHERE addr LIKE '%강남%'
...
WHERE (A.dept_cd, A.birth_de) IN (
                        ...
                        GROUP BY dept_cd
                        HAVING COUNT(*) >= 2
                        -- tb_emp를 조회해서 부서별로 정렬
                        -- 2명이상인 부서에서 제일 연장자를 조회하는 것.
                    )
ORDER BY A.emp_no
;
...
SELECT
    E.emp_no, E.emp_nm, -- 타 테이블에서 1개만 가져올 때 굳이 JOIN 할 필요가 없을 때 사용한다.
    (SELECT dept_nm FROM tb_dept D WHERE E.dept_cd = D.dept_cd) AS dept_nm, E.birth_de,
    E.sex_cd
FROM tb_emp E 
;